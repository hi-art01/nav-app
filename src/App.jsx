...
function Navigator() {
  ...
  const [encUnit, setEncUnit] = useState('meters') // 'meters' or 'feet'
  ...
  useEffect(() => {
      ...
      const EncLayer = L.GridLayer.extend({
        createTile: function (coords, done) {
          ...
          const queryObj = {
            ...
            customParameters: JSON.stringify({ DisplayDepthUnits: encUnit === 'feet' ? 2 : 1 })
          }
    ...
    setNotice(satellite ? 'Satellite imagery enabled' : encChart ? `NOAA ENC chart (${encUnit}) enabled — official navigational chart symbology` : 'NOAA chart enabled — zoom in for numeric depth soundings')
  }, [mapStyle, encUnit])
    ...
    <section className="map-area">
      ...
      <div className="map-top">
        <div>
            ...
            <p>{followHeading ? `HEADING ${Math.round(heading)}° · COMPASS FOLLOWING` : mapStyle === 'satellite' ? 'Live marine overview' : mapStyle === 'enc' ? `Electronic Navigational Chart (${encUnit === 'meters' ? 'meters' : 'feet'}) · official NOAA symbology` : 'NOAA ENC depth soundings · contours · seamarks'}</p>
        </div>
        <div className="map-top-buttons">
          <button className="map-style-button" onClick={toggleMapStyle}>
            {mapStyle === 'satellite' ? '◈ Depth chart' : mapStyle === 'nautical' ? '⌘ NOAA ENC' : '▣ Satellite view'}
          </button>
          {mapStyle === 'enc' && (
            <button className="map-unit-button" onClick={() => setEncUnit((u) => u === 'meters' ? 'feet' : 'meters')}>
              {encUnit === 'meters' ? '⚓ Switch to Feet' : '⚓ Switch to Meters'}
            </button>
          )}
        </div>
      ...
      {/* Sounding key shown when nautical chart is active */}
        ...
        {mapStyle === 'enc' && (
          <div className="depth-legend-cmap">
            <p>NOAA ENC ({encUnit === 'meters' ? 'meters' : 'feet'})</p>
            <div className="sounding-key-row"><span className="sk-num">14</span><span className="sk-label">Charted sounding ({encUnit === 'meters' ? 'm' : 'ft'})</span></div>
